...
import MembersDataTable from '@/components/shadcn-studio/blocks/database-adminAllMembers-client'
...
const DataTablePreview = async () => {
  const [language, members] = await Promise.all([getDashboardLanguage(), fetchMembersForAdmin()])

  return (
    <div className='py-8 sm:py-10'>
      <div className='max-w-9xl mx-auto px-2 sm:px-6 lg:px-8'>
        <Card className='max-w-9xl mx-auto w-full py-0'>
          <MembersDataTable data={members} language={language} />
        </Card>
      </div>
    </div>
  )
